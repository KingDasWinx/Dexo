...
`dexo mcp config print` emits a client snippet. Audit logs stay local and sanitized. A profile may allow multiple saved connections; each database-addressed call names a connection unless the profile has exactly one.
