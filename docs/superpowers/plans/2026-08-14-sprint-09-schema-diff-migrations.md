...
- [x] **Step 1:** Add failing round-trip test with common and driver-specific attributes plus SHA-256 content digest.
...
- [x] **Step 1:** Add goldens proving ordering noise is removed while PostgreSQL policy/MySQL collation differences remain.
...
- [x] **Step 1:** Add tests for add/remove/alter and ambiguous similar objects. Assert ambiguous rename remains remove+add until user supplies mapping.
...
- [x] **Step 1:** Add graph test table-before-FK on create, FK-before-table on drop, and two-view cycle marked manual.
...
- [x] **Step 1:** Add PostgreSQL/MySQL golden scripts including irreversible drop, data-loss type change, lock warning and manual cycle marker.
...
- [x] **Step 1:** Add CLI goldens for `dexo schema snapshot` and `dexo schema diff --from ... --to ... --format json|sql`; TUI snapshots for filters/risk/script.
...
- [x] Snapshot format/digest is stable and tamper-detected.
- [x] Normalization preserves native differences.
- [x] Cycles and ambiguous renames require human resolution.
- [x] Forward script reaches zero diff for fixtures.
- [x] Apply remains explicit and protected.
