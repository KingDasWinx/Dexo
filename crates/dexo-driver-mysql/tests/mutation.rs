...
async fn mysql_paging_and_typed_filter() {
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn mysql_mutation_conflict_commits_zero() {
    let fixture = connect().await;
    let data = fixture.session.data().unwrap();
    let table = QualifiedName::new(Some("dexo"), None::<String>, "items");
    drain(
        fixture
            .session
            .execute(dexo_driver_api::QueryRequest::write(
    ...
    assert!(matches!(
        page.rows[0][0],
        DbValue::I64(99) | DbValue::U64(99)
    ));
}

#[tokio::test]
#[ignore = "requires Docker"]
async fn mysql_batch_insert_update_delete() {
    let fixture = connect().await;
    let data = fixture.session.data().unwrap();
    let table = QualifiedName::new(Some("dexo"), None::<String>, "items");
    data.apply(&[
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn mysql_partial_failure_rolls_back() {
    let fixture = connect().await;
    let data = fixture.session.data().unwrap();
    let table = QualifiedName::new(Some("dexo"), None::<String>, "items");
    let error = data
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn mysql_empty_apply_is_cancel() {
    let fixture = connect().await;
    let data = fixture.session.data().unwrap();
...
#[tokio::test]
#[ignore = "requires Docker"]
async fn mysql_bulk_insert_batch() {
    let fixture = connect().await;
